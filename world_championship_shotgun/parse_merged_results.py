#!/usr/bin/env python3
"""Parse merged Shotgun World Championship result PDFs into compact rows.
...
DEFAULT_INPUT_DIR = Path("world_championship_shotgun/merged")
DEFAULT_OUTPUT_CSV = Path("world_championship_shotgun/parsed/world_championship_shotgun_results.csv")
DEFAULT_OUTPUT_JSONL = Path("world_championship_shotgun/parsed/world_championship_shotgun_results.jsonl")
OUTPUT_FIELDS = [
    "championship_name",
    "event__name",
    "round_name",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Parse shotgun merged result PDFs into compact result rows."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    parser.add_argument(
        "--no-ocr",
        action="store_true",
        help="Do not OCR pages that have no embedded text.",
    )
...
def championship_name(pdf_path: Path) -> str:
    ...
    if match:
        return f"{match.group(1)} Shotgun World Championship"
    return "Shotgun World Championship"
...
def ocr_page_lines(pdf_path: Path, page_number: int) -> list[str]:
    ...
    with tempfile.TemporaryDirectory(prefix="shotgun_ocr_") as temp_dir:
...
def page_event_round(lines: list[str]) -> tuple[str | None, str | None]:
    ...
    for line in lines[:12]:
        ...
        if upper in ROUND_KEYWORDS or any(keyword in upper for keyword in ROUND_KEYWORDS):
    ...
    if event and round_name == "Team" and "Mixed Team" in event:
        round_name = "Qualification"
    return event, round_name


def infer_event_round_from_report_code(lines: list[str]) -> tuple[str | None, str | None]:
    joined = " ".join(lines)
    code_map = {
        "TRMTJ": "Trap Mixed Team Junior",
        "TRTEAMMJ": "Trap Team Men Junior",
        "TRTEAMM": "Trap Team Men",
        "TRTEAMWJ": "Trap Team Women Junior",
        "TRTEAMW": "Trap Team Women",
        "TRMIX": "Trap Mixed Team",
        "SKMT": "Skeet Mixed Team",
        "SKM": "Skeet Men",
        "SKW": "Skeet Women",
        "TRM": "Trap Men",
        "TRW": "Trap Women",
    }
    for code, event in code_map.items():
        if code in joined:
            round_name = "Qualification" if re.search(r"\bQ\d", joined) else None
            if re.search(r"\bG\d", joined):
                round_name = "Gold Medal Match"
            elif re.search(r"\bB\d", joined):
                round_name = "Bronze Medal Match"
            return event, round_name
    return None, None


def fallback_event_round_from_filename(pdf_path: Path) -> tuple[str | None, str | None]:
    stem = pdf_path.stem.lower()
    if "mixed" in stem:
        return "Trap Mixed Team", "Qualification"
    if "womens" in stem:
        return "Trap Women", "Qualification"
    if "mens" in stem:
        return "Trap Men", "Qualification"
    return None, None
...
def score_tokens(text: str) -> list[str]:
    ...
    return [
        ...
        for token in re.findall(
            r"\b(?:DNS|DNF|DSQ|[\d]+(?:\.\d+)?(?:-\s*\d+x)?(?:\+\d+)*)(?=[A-Za-z]|\b)",
            text,
        )
    ]
...
def is_noise(line: str) -> bool:
    ...
    if re.match(r"^[1-9]\s+[2-9]\s+[3-9]", line):
        return True
    if "Version of" in line or "Release:" in line:
        return True
    if any(token in line for token in ("REPUBLIKA", "AUTOZUBAK", "BUG", "osiex")):
        return True
    return False


def is_section_end(line: str) -> bool:
    return line.startswith(("Legend", "History", "Release:", "Version of", "Summary", "Number of athletes"))


def is_probable_unbibbed_athlete(line: str) -> bool:
    cleaned = line.replace("|", "I")
    if "DNS" in cleaned or " - " in cleaned:
        return False
...
def event_is_team(event: str | None) -> bool:
    return bool(event and "Team" in event)
...
def individual_glued_result_match(line: str) -> re.Match[str] | None:
    return re.match(
        r"^(?P<rank>\d{1,3})\s+(?P<bib>\d{3,4})\s*"
        r"(?P<name>[A-Z][A-Z' -]+?)\s+(?P<noc>[A-Z]{3})\s+"
        r"(?P<scores>.+)$",
        line,
    )
...
def split_name_noc_scores(rest: str) -> tuple[str, str | None, str]:
    ...
    noisy_gap = re.match(
        r"(?P<name>.+\s.+)\s+(?P<noc>[A-Z]{3})[~\s]+(?P<scores>(?:DNS|DNF|DSQ|[-\d]).*)$",
        rest,
    )
    if noisy_gap:
        return (
            compact(noisy_gap.group("name")),
            noisy_gap.group("noc"),
            compact(noisy_gap.group("scores")),
        )
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_number, page in enumerate(pdf.pages, start=1):
            ...
            if not lines and use_ocr:
                lines = ocr_page_lines(pdf_path, page_number)
            if not lines:
                continue
            ...
            if (
                not event
                and active_event
                and any(line.startswith("Rounds") for line in lines[:6])
            ):
                event, round_name = active_event, active_round
            if not event:
                inferred_event, inferred_round = infer_event_round_from_report_code(lines)
                if (
                    active_event
                    and "Mixed Team" in active_event
                    and inferred_event in {"Trap Men", "Trap Women", "Skeet Men", "Skeet Women"}
                    and not inferred_round
                ):
                    event, round_name = active_event, active_round
                else:
                    event, round_name = inferred_event, inferred_round
            if not event:
                event, round_name = active_event, active_round
            if not event:
                event, round_name = fallback_event_round_from_filename(pdf_path)
            if event and not round_name:
                if active_event == event and active_round:
                    round_name = active_round
                elif "QUALIFICATION" in " ".join(lines[:12]).upper() or any(
                    line.startswith("Rounds") for line in lines[:6]
                ):
                    round_name = "Qualification"
            if event:
                active_event = event
            ...
            for line in lines:
                ...
                if team_medal and (event_is_team(event) or "TEAM" in " ".join(lines[:8]).upper()):
                    current_team = {
                ...
                if current_team and not re.match(r"^\d", line):
                    ...
                    if not is_probable_unbibbed_athlete(line):
                        continue
                    records.append(
                        row(
                            championship,
                ...
                glued = individual_glued_result_match(line)
                if glued:
                    current_team = None
                    parsed = row(
                        championship,
                        ...
                        glued.group("rank"),
                        glued.group("bib"),
                        compact(glued.group("name").title()),
                        glued.group("noc"),
                        last_score(glued.group("scores")),
                    )
