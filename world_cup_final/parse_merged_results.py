#!/usr/bin/env python3
"""Parse merged World Cup Final result PDFs into compact CSV/JSONL rows.
...
DEFAULT_INPUT_DIR = Path("world_cup_final/merged")
DEFAULT_OUTPUT_CSV = Path("world_cup_final/parsed/world_cup_final_results.csv")
DEFAULT_OUTPUT_JSONL = Path("world_cup_final/parsed/world_cup_final_results.jsonl")
OUTPUT_FIELDS = [
    "championship_name",
    "event__name",
    "round_name",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Parse all PDFs in world_cup_final/merged into compact result rows."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
...
def championship_name(pdf_path: Path) -> str:
    ...
    if match:
        return f"{match.group(1)} World Cup Final"
